				...
				<div class="flex flex-row space-x-3 py-1 rounded-xl border">
					...
					<div class="flex group w-full">
						<button @click="runConn(token)" type="button" class="p-3 pb-0 text-gray-400 hover:text-main-500 w-full">
							<span class="flex flex-col items-center">
                                <svg v-if="runningConn" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 fill-current animate-spin text-gray-500 group-hover:text-gray-700 transition-color duration-200" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 24 24">
                                ...
                                <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 fill-current text-gray-500 group-hover:text-gray-700 transition-color duration-200" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 24 24">
...
export default {
    ...
    methods: {
        async runConn(token) {
            this.runningConn = true;
            await this.$store.dispatch('run/updateConnection', {token: token});
            this.runningConn = false;
